...
| `GET /Videos/{id}/master.m3u8?videoCodec=h264,hevc&videoRangeType=SDR&maxVideoBitDepth=8&...&api_key=<token>` | **Always** used for playback — Jellyfin's adaptive HLS direct-streams codecs `AVPlayer` can decode and transcodes anything it can't (e.g. MPEG-2, VC-1). Static direct-play (`stream?static=true`) is intentionally **not used**: Apple devices have no MPEG-2 decoder, so it silently fails on that content. `videoRangeType=SDR` + `maxVideoBitDepth=8` force any HDR10/Dolby Vision source down to plain 8-bit SDR: without it, a 10-bit HEVC source direct-streams as-is (still an "allowed" codec) and tvOS silently refuses to render that video layer — audio and the scrub bar keep going, picture is just black. Not a DRM/licensing issue (Jellyfin content here carries no DRM); it's tvOS's own output-protection policy for wide-gamut video. |
