...
Health check: `GET http://localhost:4000/api/v1/health` -> `{ "ok": true, "version": "1.0.42" }`.
`version` is this server's own version — read from `package.json`, bumped by
`scripts/build-ipa.sh` on every push (same commit-count counter as the tvOS app's build
number). Shown in the admin dashboard's sidebar footer and in the startup log line.
...
| GET    | `/admin/app-version`            | `{latestVersion,counts}` — read-only, generated by `scripts/build-ipa.sh` on every push (`latest-app-version.json`), no PUT |
