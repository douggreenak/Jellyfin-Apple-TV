...
// No PUT here by design — latest-app-version.json is generated on push
// alongside the IPA (see build-ipa.sh) and read straight off disk. Read the
// same file ourselves so this test works whether or not a build has ever
// been generated in this checkout.
let realLatestVersion = null;
try {
  const here = dirname(fileURLToPath(import.meta.url));
  realLatestVersion = JSON.parse(readFileSync(join(here, "latest-app-version.json"), "utf8")).version;
} catch {
  // No file yet (fresh checkout that hasn't run build-ipa.sh) — every unit
  // should read "unknown" below, which the branch handles.
}

r = await fetch(`${BASE}/admin/app-version`, { headers: ah });
const av = await j(r);
check("get latest version matches generated file", av.latestVersion === realLatestVersion, JSON.stringify(av));
