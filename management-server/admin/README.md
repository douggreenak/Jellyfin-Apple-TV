...
- **Units** (`/`) — Live grid of every registered Apple TV. Polls every 2s (matching the
  fleet's 3s heartbeat) and shows online status, last-seen, model / tvOS / app version
  (flagged **outdated** against the version generated by the last push — see
  `docs/ARCHITECTURE.md` §2, "App version tracking"), what's playing, and quick
  **Identify** / **Reload** actions, plus **bulk fleet actions** (adopt/unadopt/reload/
  identify/restart/delete/migrate) over a multi-selection. A device only shows as
  **ready to adopt** while it's actually online — one that goes offline before anyone
  adopts it just disappears; offline tracking is reserved for units already adopted.
- **Unit detail** (`/units/:id`) — Tabbed editor: **General** (name, group, telemetry,
  ...
  lists discovered libraries), **Appearance** (app title, theme, poster style, clock/titles
  toggles — accent color is intentionally not here, see below), **Browse** (mode + library
  pickers, including a folder-lock tree fed by the Jellyfin browse/resolve endpoints),
  **Playback**, and a **remote control + power** panel (pair the unit's physical Apple TV
  over pyatv, then send remote-control key presses or turn it on/off). A sticky save bar
  PATCHes only the fields you changed and confirms with a snackbar.
- **Defaults** (`/defaults`) — Edit the template applied to newly registered TVs, including a
  **"push to all"** action that re-sends just the Jellyfin account to every existing unit.
  (There is no app-version field here — that's generated automatically on every push, not
  admin-editable; see `docs/ARCHITECTURE.md` §2.)
...
**Dashboard theme color**: the palette icon next to the dark/light toggle (top bar) picks
*this dashboard's own* accent color — a per-browser preference in `localStorage`
(`admin-theme-accent`), unrelated to any TV setting. TVs always use their fixed default
accent color; there is no per-fleet color configurable from this admin UI.
...
- `GET /admin/app-version` (read-only — generated by `scripts/build-ipa.sh`, no PUT)
